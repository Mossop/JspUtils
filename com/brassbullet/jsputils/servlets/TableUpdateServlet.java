...
import com.brassbullet.dbutils.xml.DatabaseParser;
import com.brassbullet.dbutils.IntegrityValidator;
    ...
    if (dbcon!=null)
    {
    	...
    	if (tablename!=null)
    	{
    	  ...
    	  if (table!=null)
    	  {
    	    ...
    	    {
    	      String oldval = request.getParameter("OLD"+table.getAllField(loop).getName());
    	      if (oldval!=null)
    	      {
    	    ...
    	    String newfieldlist = "UPDATE "+tablename+" SET ";
    	    for (int loop=0; loop<table.getAllFieldCount(); loop++)
    	    {
    	      String newval = request.getParameter(table.getAllField(loop).getName());
    	      if (newval!=null)
    	      {
    	        fieldlist=fieldlist+table.getAllField(loop).getName()+",";
    	        newfieldlist=newfieldlist+table.getAllField(loop).getName()+"=";
    	        if (table.getAllField(loop).isString())
    	        {
    	          newfieldlist=newfieldlist+"'"+newval+"',";
    	        }
    	        else
    	        {
    	          newfieldlist=newfieldlist+newval+",";
    	    ...
    	    querylist.addElement(newfieldlist+" "+wherelist);
    	    boolean valid=true;
    	    try
    	    {
    	      ...
    	      while (results.next())
    	      {
    	        ...
    	        for (int loop=0; loop<table.getAllFieldCount(); loop++)
    	        {
    	          ...
    	          String newval = request.getParameter(current.getName());
    	          if (newval!=null)
    	          {
    	            if (current.isString())
    	            {
    	              if (!validator.validateModify(querylist,current,"'"+results.getString(pos)+"'","'"+newval+"'"))
    	              {
    	                valid=false;
    	              }
    	            }
